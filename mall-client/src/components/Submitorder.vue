    ...
    <div class="submitorder">
      ...
      <div class="checkshopchat">
        ...
        <div class="order-order">
          <div class="order-item" v-for="(item,index) in orderDetail" :key="index">
            <div class="order-itemInfo">
              ...
              <div class="info-price info-cell">{{item.retailprice}}</div>
            </div>
            <div class="order-quantity">{{item.amount}}</div>
            <div class="order-integral">{{item.retailprice*item.amount}}</div>
...
export default {
  ...
  computed: {
    // 计算购物车不算折扣的时候多少钱
    idealistictotal(){
      return this.orderDetail.reduce((pre,next) => {
    ...
    total(){
      if(!this.flag){ // 未选择优惠
        ...
        return this.orderDetail.reduce((pre,next) => {
          return pre + next.retailprice * next.amount;
        },-this.discount) // -this.discount为pre初始的默认值
      }
  ...
  methods: {
    ...
    getShopList(){
      instance.get('/client/shopchatlist').then(response => {
        ...
        this.orderDetail = response.data.shopchatList;
        this.thisdiscountintegral = response.data.searchintegral[0].integral; // 能用多少积分
        this.discount = response.data.searchintegral[0].integral/100; // 该积分可以抵消多少钱
      })
    },
    // 提交订单成功之后清空购物车
    emptyShopList(){
      let arr = this.orderDetail; //购物车商品列表
      let emptyproducts = []; // 存储需要清空的购物车商品的id
      arr.forEach((value,i) => {
        emptyproducts.push(value.id)
      })
      instance.post('/client/emptyshoplist',{
    ...
    getAddress(){
      instance.get('/client/addresslist').then(response => {
        ...
        arr.forEach((value,i) => {
          ...
          this.arrry.push(addressObject);          
        })
        this.addressList = this.arrry;
    ...
    addAddress(){
      ...
      // arr.push(this.form.detailaddress)
      let str=arr.join(' ')
    ...
    submitOrder(){
      if(this.value){ // 地址已选的时候
        ...
        instance.post('/client/submitorder',{
          ...
          if(response.data.code === 1 && response.data.msg === 'success'){
            this.emptyShopList(); //提交订单成功之后清空购物车
            this.$message({
              message: '订单提交成功',
              type: 'success'
            });
