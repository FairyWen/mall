<template>
  <div id="allorder">
    ...
    <div class="order-order">
      <div class="order-item" v-for="(item,index) in allOrderList" :key="index">
        ...
        <div class="order-action td-5">
          <el-link type="primary" @click="toAction(item.id,item.state)">{{item.action}}</el-link>
...
export default {
  ...
  methods: {
    // 获取所有订单列表
    getAllOrderList(){
      instance.get('/client/allorderlist').then(response => {
    ...
    toDeliver(id){
      this.updateLoading = true;
      let api = "/apis/client/remindshipment";
      Axios.post(api, {
        order_id: id,
      }).then(response => {
        if (response.data.code === 1 && response.data.msg === "success") {
    ...
    toReceive(id){
      this.updateLoading = true;
      let api = "/apis/client/confirmreceipt";
      Axios.post(api, {
        order_id: id,
      }).then(response => {
        if (response.data.code === 1 && response.data.msg === "success") {
    ...
    toEvaluate(id){
      this.updateLoading = true;
      let api = "/apis/client/toevaluate";
      Axios.post(api, {
        order_id: id,
      }).then(response => {
        if (response.data.code === 1 && response.data.msg === "success") {
