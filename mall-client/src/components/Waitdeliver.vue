  ...
  <div id="allorder">
    <div class="order-desc">
      ...
      <div class="order-order">
        <div class="order-item" v-for="(item,index) in waitDeliverList" :key="index">
          ...
          <div class="order-action td-5">
            <el-link type="primary" @click="toDeliver(item.id)">{{item.action}}</el-link>
...
export default {
  ...
  methods: {
    // 获取待收货订单列表
    getWaitDeliverList(){
      instance.get('/client/waitdeliverlist').then(response => {
        this.waitDeliverList = response.data;
      })
    },
    // 查看订单详情
    seeOrderDetail(id){
