  ...
  <div id="allorder">
    <div class="order-desc">
      ...
      <div class="order-order">
        <div class="order-item" v-for="(item,index) in waitReceiveList" :key="index">
          ...
          <div class="order-action td-5">
            <el-link type="primary" @click="toReceive(item.id)">{{item.action}}</el-link>
...
export default {
  ...
  methods: {
    // 获取待收货订单列表
    getWaitReceiveList(){
      instance.get('/client/waitreceivelist').then(response => {
        this.waitReceiveList = response.data;
      })
    },
    // 查看订单详情
    seeOrderDetail(id){
